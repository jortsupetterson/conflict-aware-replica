...
for (let i = 0; i < OPS; i++) writer.title = `t${i}`;
await writer.flush();
const opBatch = emitted.slice();

const verifier = await Dacument.load({
  schema,
  roleKey: roleKeys.owner.privateKey,
  snapshot,
});
await verifier.merge(opBatch);
const verifySnapshot = verifier.snapshot();
...
console.log(`Dacument actor integrity bench (runs=${RUNS}, ops=${OPS})`);

await benchAsync("merge (actor verify disabled)", async () => {
...
await benchAsync("verifyActorIntegrity", async () => {
  const start = performance.now();
  await verifier.verifyActorIntegrity({ snapshot: verifySnapshot });
  return performance.now() - start;
});
