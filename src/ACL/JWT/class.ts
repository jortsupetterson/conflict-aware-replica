...
import { Identifier, Role } from "../class";

export class JWT {
  private header = { alg: "ES256", typ: "JWT" };
  private payload: {
    iss: Identifier;
    sub: Identifier;
    iat: number;
  ...
  static async sign(privateJwk: JsonWebKey, jwt: JWT) {
  ...
  static async verify(publicJwk: JsonWebKey, token: string) {
    try {
